...
func `[]`*(
    node: TsFennelNode,
    idx:  int,
    kind: FennelNodeKind | set[FennelNodeKind]
  ): TsFennelNode =
...
proc treeReprTsFennel*(str: string, unnamed: bool = false): ColoredText =
  treeRepr[TsFennelNode, FennelNodeKind](parseTsFennelString(str), str, 6, unnamed = unnamed)

proc toHtsNode*(
    node: TsFennelNode,
    str:  ptr string
  ): HtsNode[TsFennelNode, FennelNodeKind] =
...
proc parseFennelString*(str: string, unnamed: bool = false): FennelNode =
  let parser = newTsFennelParser()
  return toHtsTree[TsFennelNode, FennelNodeKind](parseString(parser, str), unsafeAddr str, storePtr = false)
